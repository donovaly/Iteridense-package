...
function Clustering(dataMatrix; minClusterSize::Int= 3, startResolution::Int= 2,
    ...
    if LoopResult.numOfClusters == 0
        ...
        return IteridenseResultC(ArrayToCTensor([]), ArrayToCTensor([]), Cint(0),
                                    Cint(LoopResult.finalResolution),
...
Base.@ccallable function IteridenseClustering()::Ptr{IteridenseResultC}
    
    # allocate memory for the uninitialized struct
    pointer = Ptr{IteridenseResultC}(malloc(sizeof(IteridenseResultC)))
    if pointer == C_NULL
        return C_NULL
    end
    ...
    unsafe_store!(pointer, result)

    return pointer
end

# function to free the memory allocated by IteridenseClustering
Base.@ccallable function IteridenseFree(pointer::Ptr{IteridenseResultC})::Cint
    if pointer == C_NULL
        return -1
    end
    free(pointer)
