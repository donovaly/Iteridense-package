...
and we get
\begin{equation}
\mathcal{O}\to\mathcal{O}\left(N+\left(R_{\mathrm{final}}-R_{\mathrm{start}}+2\right)ND+\frac{3^{D}\left(\left(R_{\mathrm{final}}+1\right)^{D+1}-R_{\mathrm{start}}^{D+1}\right)}{2(D+1)}\right)
\end{equation}
\begin{equation}
\mathcal{O}\approx\mathcal{O}\left(\left(R_{\mathrm{final}}-R_{\mathrm{start}}+2\right)ND+3^{D}\left(\left(R_{\mathrm{final}}+1\right)^{D+1}-R_{\mathrm{start}}^{D+1}\right)\right)\label{eq:O()}
\end{equation}

In the naive approach, $R_{\mathrm{start}}=2$ as this is the lowest
possible resolution we have
\begin{equation}
\mathcal{O}_{\mathrm{worst\,case}}\approx\mathcal{O}\left(R_{\mathrm{final}}ND+3^{D}\left(R_{\mathrm{final}}+1\right)^{D+1}\right)\label{eq:O(worst)}
\end{equation}
...
\midrule
\midrule 
Iteridense & $\mathcal{O}\left(DN\right)$ & actually formula \eqref{eq:O()}\tabularnewline
\midrule 
...
took $\approx4.5\thinspace$s\footnote{The times were measured using the feature \emph{@elapsed} of the \noun{Julia}
programming language.} and the final resolution was 13. For $\rho=2.0$ the final resolution
was 25 and with $\approx11.7\thinspace$s more time was needed. By
starting the algorithm at a higher resolution, e.\,g. with \textbf{StartResolution~}=~10
the time decreases from $\approx4.5\thinspace$s to $\approx3.7\thinspace$s
and from $\approx11.7\thinspace$s to $\approx10.5\thinspace$s, respectively.
In effect the maximal calculated resolution determines the runtime.
...
runs took $\approx12.1\thinspace$s while the same with $\epsilon=0.5$
took $\approx49\thinspace$s.\footnote{The computation time measurement of the DBSCAN algorithm was performed
using its implementation in the package \emph{Clustering} of the \noun{Julia}
programming language~\cite{ClusteringDBSCAN}.\\
With $\epsilon=0.5$ both clusters were not detected.} Therefore setting $\epsilon$ to a suitable value does not only affect
...
dimensions taken into account.]{\includegraphics[width=0.33\columnwidth]{clipart/Pluto-Iteridense-rho2-5-only3}

}\subfloat[Clustering with all dimensions taken into account.]{\includegraphics[width=0.33\columnwidth]{clipart/Pluto-Iteridense-rho2-5-all}

}\subfloat[like (b) but $\rho=5.9$]{\includegraphics[width=0.33\columnwidth]{clipart/Pluto-Iteridense-rho5-9-all}
