...
\IEEEauthorblockA{Soloof EOOD, Plovdiv, Bulgaria\\
...
combining grid-based and density-based methods. It provides two ways
...
results. We show that \noun{Iteridense} provides lower computational
complexity than pure density-based algorithms and that it performs
clustering at least as good as the DBSCAN algorithm. Our algorithm
is deterministic, does not forcibly assign all data points to a cluster
and optionally allows to specify the number of clusters. We provide
a reference implementation of \noun{Iteridense} as well as a stand-alone
...
practical applications. Therefore a variety of algorithms exists~\cite{Kriegel2011}.
However, many of these algorithms have as major drawback that one
needs to evaluate neighboring points for every data point in the dataset
to determine if it is part of a cluster. This is the case for most
density-based clustering algorithms like PreDeCon~\cite{Boehm2004}
or HDBSCAN~\cite{Campello2013}. Other algorithms that combine grid-based
with density-based methods like DENCLUE~\cite{Hinneburg1998} or
CLIQUE\,\cite{Agrawal1998} don't have this drawback but make assumptions
about the shape of the probability distribution of the data (what
is the probability to find a data point inside the range of available
data).

Another issue of many clustering algorithms is that there is no clear
path for the user on how to change the algorithm's input parameters
...
a suitable result. We will discuss this particular case and how our
algorithm is a benefit for the user.
...
leads to a lower computational complexity than for pure density-based
algorithms like DBSCAN. Our algorithm follows an iterative approach
to calculate the probability-density function and makes no assumptions
about the shape of that function. Therefore it provides a clear path
on how to set a start value of the algorithm's main parameter and
how to change it to achieve a certain result.

The algorithm clusters in a way that no point in a dataset can belong
to more than one cluster, and that points in the dataset are not forcibly
...
from each other in a topographical relief. Take for example the relief
...
Motivation} in \cite{Kriegel2011} for a similar visualization than
\ref{fig:Relief-of-a}.
...
point, making the clustering computation-efficient, as discussed in
\ref{subsec:Computational-Complexity}. The clusters are derived by
counting the numbers of data points within a certain data space.
...
least be detected. The possibility to specify \textbf{MinClusters}
...
a whole 2-dimensional dataset has $\rho=1$. Because of the iteration
with increasing resolutions our algorithm provides a clear path on
how to set and change $\rho$: Start with a low $\rho>1$ and increase
$\rho$ until you get a suitable result. The algorithm stops at the
resolution at which the specification of $\rho$ is fulfilled. We
will demonstrate this feature with an example in \ref{subsec:Clustering-Performance}.
\end{itemize}
...
or the minimal density $\rho$ every found cluster must have. $\rho$
...
evaluation scheme is depicted in \ref{fig:Clustering-process}\,(c).
...
in cluster ``0'' are hereby not part of any cluster.
...
\item \label{enu:OptionOmitEmptyCells}In step~\ref{enu:AlgoStep1} don't
divide the data space in $\text{resolution}^{\text{dimension}}$ cells
but use for every dimension only as many cells as necessary (\textbf{OmitEmptyCells}).
\end{enumerate}
...
it might lead to unsuitable results. Therefore this option should
only be used if really desired.
...
Option~\ref{enu:OptionOmitEmptyCells} is important for high dimensions
to make it possible to perform the clustering, see \ref{subsec:Memory-Complexity}
for details.

A reference implementation of \noun{Iteridense} in the programming
language \noun{Julia} is online available~\cite{Iteridense}. Its
outputs are the assignments of the points to clusters, size of clusters,
density of clusters, final resolution, number of clusters, count map
as tensor in the final resolution (the actual probability-density
...
$C=2$, thus $\rho_{\mathrm{cluster}}=4$. We look at these 4~cases:
\begin{itemize}
...
The data shown in \ref{fig:Result-Moons} and \ref{fig:Effect-of-a}
was generated using the \emph{make\_moons }call to \emph{sklearn.datasets}.
\ref{fig:Result-Moons}\,(a) shows the result for $\rho=2.2$, \ref{fig:Result-Moons}\,(b)
...
\par\end{centering}
\caption{\label{fig:Result-Moons}Result of \noun{Iteridense} for intersected
...
$\rho$ \ref{fig:Result-Iteridense-Anisotropic}\,(a)\,--\,\ref{fig:Result-Iteridense-Anisotropic2}\,(a)
...
\par\end{centering}
\caption{\label{fig:Result-Iteridense-Anisotropic}Result of \noun{Iteridense}
for $\rho\ge6.8$ at anisotropic clusters.}
\end{figure}
...
\par\end{centering}
\caption{\label{fig:Result-Iteridense-Anisotropic2}Result of \noun{Iteridense}
for $\rho=12.0$ and DBSCAN for $\epsilon=0.1$ at anisotropic clusters.}
\end{figure}
...
in \ref{fig:Result-Iteridense-Anisotropic2}\,(b). This data was
generated using the \emph{make\_blobs }call to \emph{sklearn.datasets}
with a subsequent transformation. Like in the previous example, there
are 1500~data points. $\epsilon=0.1$ (the maximum distance to another
...
data shown in \ref{fig:Result-Different-Density-Clusters} consist
...
to 2 and \textbf{MinClusterSize} to 20. As result one gets \ref{fig:Result-Different-Density-Clusters}\,(a).
...
density and one with a lower density, see \ref{fig:Result-Different-Density-Clusters}\,(b).
\item Or increase $\rho$ and also increase \textbf{MinClusterSize} e.g.~to
50 unless one gets only 2~high-density clusters, see \ref{fig:Result-Different-Density-Clusters}\,(c).
...
\par\end{centering}
\caption{\label{fig:Result-Different-Density-Clusters}Result of \noun{Iteridense}
...
is different because DENCLUE assumes a Gaussian distribution function
as shape of the density function. Another big difference to DENCLUE
is that there is not only a single probability-density function created
but iteratively several ones with increasing resolution. This increases
the computation efforts for the benefit that one does not need to
make assumptions about the density in the dataset. DENCLUE requires
at least 2~input variables, $\xi$ and $\sigma_{D}$. $\xi$ is similar
to $\rho$ of \noun{Iteridense}, $\sigma_{D}$ defines the width of
the Gaussian that is used to assign the data points to the clusters.
Its value has to be guessed and therefore introduces for some practical
applications a trial and error process. There are approaches to improve
the initial setting of the DENCLUE parameters, \cite{Ajmal2024} but
in general this will remain a practical challenge.
...
He can either specify \textbf{MinClusters} or $\rho$ , sets \textbf{MinClusterSize}
and gets in many cases directly a suitable result. \noun{Iteridense}'s
...
The author thanks Michael Klaiber, Ph.D for the fruitful discussion
and feedback and for the scientific proof-reading.

\subsection*{Ethical Statement}

The author works for the solar company Soloof. He and Soloof got no
financial support for this work. Soloof as a solar company focuses
in solar solutions and \noun{Iteridense} is a side-product of their
work they share with the public.

\appendix{}

By design \noun{Iteridense} delivers clustering results for any possible
resolution. The possible resolutions are in the range 2\,--\,number
of data points. The final clustering result is at the resolution at
which the specified values for either $\rho$ or \textbf{MinClusters}
are reached. The question is why we don't use the resolution as a
parameter to stop the loop of the \noun{Iteridense} algorithm. To
answer this, we have to look how the clustering result changes with
the resolution and how this change behave for different data sets.

\ref{fig:Dataset-with-random}\,(b) shows the number of clusters
in dependence of the resolution for some of the data sets used in
this paper. For comparison also a the set of random data points was
clustered. All used datasets have each 1500~data points. \textbf{MinClusterSize}
was in all cases set to 75 (5\,\% of the number of data points).
\ref{fig:Mean-size-and-density}\,(a) shows the mean size of clusters
in dependence of the resolution and \ref{fig:Mean-size-and-density}\,(b)
shows the mean density of clusters in dependence of the resolution.

\begin{figure}
\begin{centering}
\hfill{}\subfloat[Dataset with random noise.]{\begin{centering}
\includegraphics[scale=0.4]{clipart/Random-data}
\par\end{centering}
}\hfill{}\subfloat[Numbers of clusters in dependence of resolution.]{\begin{centering}
\includegraphics[scale=0.4]{clipart/Resolution-NumClusters}
\par\end{centering}
}\hfill{}
\par\end{centering}
\caption{\label{fig:Dataset-with-random}Dataset with random data and numbers
of clusters in dependence of the resolution for:\protect \\
- moon-like clusters shown in \ref{fig:Result-Moons}\protect \\
- anisotropic clusters shown in \ref{fig:Result-Iteridense-Anisotropic}\protect \\
- different-density clusters shown in \ref{fig:Result-Different-Density-Clusters}\protect \\
- circle-like clusters shown in \ref{fig:Result-Iteridense-Circles}\protect \\
- random noise shown in \ref{fig:Dataset-with-random}\,(a).}
\end{figure}

\begin{figure}
\begin{centering}
\hfill{}\subfloat[Mean size of clusters in dependence of the resolution.]{\begin{centering}
\includegraphics[scale=0.4]{clipart/Resolution-ClusterSize}
\par\end{centering}
}\hfill{}\subfloat[Mean density of clusters in dependence of the resolution.]{\begin{centering}
\includegraphics[scale=0.4]{clipart/Resolution-ClusterDensity}
\par\end{centering}
}\hfill{}
\par\end{centering}
\caption{\label{fig:Mean-size-and-density}Mean size and mean density of clusters
in dependence of the resolution using the same data as in \ref{fig:Dataset-with-random}\,(b).}
\end{figure}

It can be seen that it is impossible to predict what resolution leads
to what clustering results. Therefore the resolution is not a suitable
parameter to stop the algorithm loop. For practical applications the
goal is to find either at least a certain amount of clusters or clusters
with at least a certain density.
