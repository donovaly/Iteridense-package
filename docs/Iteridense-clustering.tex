...
\begin{equation}
\rho_{\mathrm{cluster\,norm}}=\frac{\text{num points in cluster}}{\text{num cells of cluster}}\cdot\frac{\text{total num of points}}{\text{total num of cells}}\cdot\frac{\text{dimension}}{2\thinspace\text{resolution}^{\text{dimension}-2}}
\end{equation}

The normalization is derived in \ref{subsec:Density-Normalization}.
For 2~dimensions the last term vanishes and $\rho_{\mathrm{cluster\,norm}}$
describes by what factor the cluster is more dense than the whole
data set.

The final density $\rho_{\mathrm{final}}$ is the minimum of all $\rho_{\mathrm{cluster\,norm}}$.\\
(In the following $\rho_{\mathrm{cluster}}$ means $\rho_{\mathrm{cluster\,norm}}$.)
...
for the algorithm. A suitable clustering result for this data set
would probably be one with 2~clusters and we will discuss what ``suitable''
means later.
...
$\rho_{\mathrm{cluster}}$ with the density of the whole data set
\begin{equation}
\rho_{\mathrm{data\,set}}=\frac{\text{total num of points}}{\text{total num of cells}}
\end{equation}
 and we would then have
\[
\rho_{\mathrm{cluster\,norm}}=\frac{\rho_{\mathrm{cluster}}}{\rho_{\mathrm{data\,set}}}
\]

This density is independent of the resolution but not on the dimension.
In practice is is often a tricky question if more or less dimensions
...
Take for example this case: $D=1$, resolution $R=8$, number of cells
of cluster $C=R/4$ ($C$ depends on $R$), number of points in cluster
$P=8$, total number of points $N=16$. We get then $\rho_{\mathrm{cluster}}={\displaystyle \frac{P}{C}}=4\vphantom{\cfrac{A^{A^{A}}}{A_{A_{A}}}_{_{A_{A}}}^{A^{A}}}$,
$\rho_{\mathrm{data\,set\,1D}}={\displaystyle \frac{N}{R}}=2\vphantom{\cfrac{A^{A^{A}}}{A_{A_{A}}}_{_{A_{A}}}^{A^{A}}}$,
$\rho_{\mathrm{cluster\,norm\,1D}}={\displaystyle \frac{PR}{CN}}=2\vphantom{\cfrac{A^{A^{A}}}{A_{A_{A}}}_{_{A_{A}}}^{A^{A}}}$,
so the cluster is in 1D 2~times more dense than the data set.
...
$\rho_{\mathrm{cluster}}={\displaystyle \frac{P}{C}}$, $\rho_{\mathrm{data\,set\,2D}}={\displaystyle \frac{2\cdot2\cdot P}{R^{2}}}$,
$\rho_{\mathrm{cluster\,norm\,2D}}={\displaystyle \frac{P\cdot R^{2}}{C\cdot2\cdot2\cdot P}}=8$,

so the cluster is 8~times more dense than the data set and $\rho_{\mathrm{cluster\,norm}}$
...
The solution is to preserve the $\rho_{\mathrm{cluster\,norm}}$ of
the 1D case to all greater dimensions using the assumptions we made
for adding a dimension. That means we have to multiply $\rho_{\mathrm{cluster\,norm}}$
...
only depends on $\rho_{\mathrm{data\,set}}$. With our assumptions
we have
\begin{equation}
\rho_{\mathrm{data\,set}}=\frac{2D\cdot P}{R^{D}}
\end{equation}
and
\begin{equation}
\gamma_{1D}=\frac{\rho_{\mathrm{data\,set\,D}}}{\rho_{\mathrm{data\,set\,1D}}}=\frac{R\cdot2D\cdot P}{2\cdot PR^{D}}=\frac{D}{R^{D-1}}
\end{equation}
and
\begin{equation}
...
However, this normalization will preserve the density from the 1D
case. For most practical use cases, one starts with 2D. Therefore
we normalize according to the 2D case:

\begin{equation}
\gamma_{2D}=\frac{\rho_{\mathrm{data\,set\,D}}}{\rho_{\mathrm{data\,set\,2D}}}=\frac{R^{2}\cdot2D\cdot P}{4\cdot PR^{D}}=\frac{D}{2R^{D-2}}
\end{equation}
...
This normalization cannot cover all cases but keeps $\rho_{\mathrm{cluster}}$
...
of the same size. Then we have $\rho_{\mathrm{data\,set\,2D}}={\displaystyle \frac{2P}{R^{2}}}\vphantom{\cfrac{A^{A^{A}}}{A_{A_{A}}}_{_{A_{A}}}^{A^{A}}}$
...
size. Then we have for $D=3$ $\rho_{\mathrm{data\,set\,3D}}={\displaystyle \frac{4P}{R^{3}}}\vphantom{\cfrac{A^{A^{A}}}{A_{A_{A}}}_{_{A_{A}}}^{A^{A}}}$
...
\begin{itemize}
\item Now a new dimension is added which only adds a single new cluster
of the same size. Then we have $\rho_{\mathrm{data\,set\,2D}}={\displaystyle \frac{4P}{R^{2}}}\vphantom{\cfrac{A^{A^{A}}}{A_{A_{A}}}_{_{A_{A}}}^{A^{A}}}$
and thus $\rho_{\mathrm{cluster\,norm\,2D}}=8$. And for $D=3$ we
have $\rho_{\mathrm{data\,set\,3D}}={\displaystyle \frac{5P}{R^{3}}}\vphantom{\cfrac{A^{A^{A}}}{A_{A_{A}}}_{_{A_{A}}}^{A^{A}}}$
...
\item Now a new dimension is added which adds two new clusters of the same
size. Then we have for $D=3$ $\rho_{\mathrm{data\,set\,3D}}={\displaystyle \frac{6P}{R^{3}}}\vphantom{\cfrac{A^{A^{A}}}{A_{A_{A}}}_{_{A_{A}}}^{A^{A}}}$
...
$\rho\ge6.8$ at anistotope clusters.}
\end{figure}
...
$\rho=12.0$ and DBSCAN for $\epsilon=0.1$ at anistotope clusters.}
\end{figure}
...
\noun{Iteridense} creates two tensors, the count tensor to could how
...
the different steps:
\begin{description}
\item [{Counting}] $\mathcal{O}\left(ND\right)$ because a computation
is performed for every data point in every dimension
...
\item [{Evaluation}] To evaluate how many clusters there are and now many
cells they have, we need to step over all cluster tensor cells, therefore
we have $\mathcal{O}\left(R^{D}\right)$ for this step
\item [{Assignment}] To assign the cluster number to every data point the
complexity is $\mathcal{O}\left(N\right)$
\end{description}
\noun{Iteridense} iteratively clusters with increasing $R=R_{\mathrm{start}}\ldots R_{\mathrm{final}}+1$.
Note that the assignment is only performed once for the final $R$.
So we have these number of computes:
\begin{align}
 & N+\sum_{R=R_{\mathrm{start}}}^{R_{\mathrm{final}}+1}\left(ND)+R^{D}\left(\frac{3^{D}}{2}+1\right)\right)\\
 & \approx N+\left(R_{\mathrm{final}}+1-R_{\mathrm{start}}+1\right)ND+\frac{3^{D}}{2}\sum_{R=R_{\mathrm{start}}}^{R_{\mathrm{final}}+1}R^{D}
\end{align}

as approximation we can write
\[
\sum_{R=R_{\mathrm{start}}}^{R_{\mathrm{final}}+1}R^{D}\approx\frac{\left(R_{\mathrm{final}}+1\right)^{D+1}-R_{\mathrm{start}}^{D+1}}{D+1}
\]
and we get
\begin{equation}
\mathcal{O}\to\mathcal{O}\left(N+\left(R_{\mathrm{final}}-R_{\mathrm{start}}+2\right)ND+\frac{3^{D}}{2(D+1)}\left(\left(R_{\mathrm{final}}+1\right)^{D+1}-R_{\mathrm{start}}^{D+1}\right)\right)
\end{equation}
\begin{equation}
\mathcal{O}\approx\mathcal{O}\left(\left(R_{\mathrm{final}}-R_{\mathrm{start}}+2\right)ND+\frac{3^{D}}{2(D+1)}\left(\left(R_{\mathrm{final}}+1\right)^{D+1}-R_{\mathrm{start}}^{D+1}\right)\right)\label{eq:O()}
\end{equation}

In the naive approach, $R_{\mathrm{start}}=2$ as this is the lowest
possible resolution we have
\begin{equation}
\mathcal{O}_{\mathrm{worst\,case}}\approx\mathcal{O}\left(R_{\mathrm{final}}ND+\frac{3^{D}}{2(D+1)}\left(R_{\mathrm{final}}+1\right)^{D+1}\right)\label{eq:O(worst)}
\end{equation}

With this result the following can be seen:
\begin{itemize}
\item $\mathcal{O}$ scales roughly with $\mathcal{O}\left(DN+R^{D+1}\right)$,
thus a linear increase plus an offset
\item Setting $R_{\mathrm{start}}$(\textbf{StartResolution}) has not the
major impact, but the final resolution
\item The worst case is that $\rho$ is set so high that the $R_{\mathrm{final}}=N$.
To prevent that this happens, the parameter \textbf{StopResolution}
can be set.
\end{itemize}
\ref{tab:Computational-and-memory} lists $\mathcal{O}$ of different
clustering algorithms. To prove that our derivation of $\mathcal{O}$
is correct, we take a data set with $N=1500$ and cluster subsequently
subsets of that data set to be able to create a plot with the computation
time over $N$. The data set we use is shown in \ref{fig:Result-Iteridense-Circles}
and was generated using the \emph{make\_circles }call to \emph{sklearn.datasets}.
...
\midrule
\midrule 
Iteridense & $\mathcal{O}\left(DN+R^{D+1}\right)$ & actually formula \eqref{eq:O()}\tabularnewline
\midrule 
...
result for the full data set for $\text{\textbf{MinClusters}}=2$.
The result is $R_{\mathrm{final}}=16$. These values were taken to
measure the calculation times. For comparison \ref{fig:Result-Iteridense-Circles}\,(b)
shows the result using the DBSCAN algorithm for $\epsilon=0.1$ and
$\text{\textbf{MinPts}}=3$. We chose for the time measurement for
comparison also $\epsilon=0.2$. The computation of the k-mean and
DBSCAN algorithms were performed using their implementation in the
package \emph{Clustering} of the \noun{Julia} programming language~\cite{ClusteringDBSCAN}.
The times were measured using the feature \emph{@elapsed} of the \noun{Julia}
programming language. To get reliable values the clustering was for
every algorithm performed 2000~time in a row and the overall time
was taken.

The result is shown in \ref{fig:Computation-time-comparison}\,(a).
Iteridense shows the same linear increase as k-means proving that
formula \eqref{eq:O(worst)} is basically correct. The slope of the
increase is relatively low. For DBSCAN in the used implementation,
one can see that is has $\mathcal{O}\left(DN^{2}\right)$. It is interesting
to see how strongly the choice of $\epsilon$ influences the computation
time

\begin{figure}
\begin{centering}
\hfill{}\subfloat[$D=2$]{\begin{centering}
\includegraphics[width=0.45\columnwidth]{clipart/Calculation-time-comparison-2D}
\par\end{centering}
}\hfill{}\subfloat[$D=3$]{\begin{centering}
\includegraphics[width=0.45\columnwidth]{clipart/Calculation-time-comparison-3D}
\par\end{centering}
}\hfill{}
\par\end{centering}
...
$N=670$. By repeating the test in 3D, we should see a shift of this
value to greater number because of the greater offset in $\mathcal{O}$
for \noun{Iteridense}. The test was performed by adding z-values to
the data set so that the clusters form cylinders. DBSCAN find the
2~clusters for the range $0.2\le\epsilon\le0.26$ and we therefore
chose the lowest possible $\epsilon=0.2$. Eventually we extended
the data set by copying its points to get in total $N=7500\thinspace$points.
The result is shown in \ref{fig:Computation-time-comparison}\,(b).
The $N$ at which \noun{Iteridense} is faster than DBSCAN is not at
about $N=3700$.

\begin{figure}
\begin{centering}
\includegraphics{clipart/Iteridense-Performance5e4}
\par\end{centering}
\caption{\label{fig:Computational-complexity-of}Computational complexity of
\noun{Iteridense} in comparison to $\mathcal{O}\left(DN^{2}\right)$. }
\end{figure}

\ref{fig:Computational-complexity-of} shows $\mathcal{O}$ of \noun{Iteridense}
in comparison to $\mathcal{O}\left(DN^{2}\right)$ to summarize what
derived already.
...
\ref{fig:Result-Iteridense-Cylinders}\,(a) shows the \noun{Iteridense}
result for the full 3D data set for $\text{\textbf{MinClusters}}=2$.
Iteridense does not find the outer cylinder to be completely a cluster.
DBSCAN can find it because it allows to change the search radius.
However, it took about 20 cluster attempts to find an $\epsilon$
that works. In 3D the data set can be visualized to find a suitable
$\epsilon$ but this is impossible in higher dimensions. In effect
DBSCAN provides for this task more flexibility and a more suitable
result for the cost that one needs to try. In comparison \noun{Iteridense}
one can set $\text{\textbf{MinClusters}}=2$ and then fine-tune with
$\rho$. This data set is a good example that every algorithm has
...
\begin{figure}
\begin{centering}
\subfloat[Evaluation of the 3~shown dimensions. final resolution: 9]{\begin{centering}
\includegraphics[width=0.45\columnwidth]{clipart/PhdPubs-Iteridense-MinClusters5-sub}
\par\end{centering}
}\hfill{}\subfloat[Evaluation of all 6~dimension. final resolution: 5]{\begin{centering}
