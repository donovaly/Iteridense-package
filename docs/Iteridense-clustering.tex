...
$N=520$. By repeating the test in 3D, there should be an increase
...
which \noun{Iteridense} is faster than DBSCAN is now at about $N=1500$.
...
$\mathcal{O}\left(DN\left(1+\log(N)\right)\right)$, which results
in a total 
\begin{equation}
\mathcal{O}\left(DN\left(2+\log(N)\right)+R^{D-D_{c}}\bar{K}^{D_{c}}\right)\label{eq:O-omitEmptyCells}
\end{equation}

Compared with formula \eqref{eq:O()}, clustering with the option
\textbf{OmitEmptyCells} will have a greater linear increase of the
computation time with increasing number of data points. This can be
seen in \ref{fig:Computational-time-6D} that shows the computation
...
\midrule
\midrule 
\noun{Iteridense} & $\mathcal{O}\left(DN+R^{D+1}\right)$ to $\mathcal{O}\left(DN\left(2+\log(N)\right)+R^{D-D_{c}}\bar{K}^{D_{c}}\right)$ & see formulas \eqref{eq:O()}, \eqref{eq:O-omitEmptyCells}\tabularnewline
\midrule 
