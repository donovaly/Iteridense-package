...
probably be one with 2~clusters and we will discuss what ``suitable''
means later.
...
so the cluster is in 1D 2~times more dense than the dataset.
...
So there are these number of computes:
\begin{align}
...
and get
\begin{equation}
\mathcal{O}\left(N+\left(R_{\mathrm{final}}-R_{\mathrm{start}}+2\right)ND+\frac{3^{D}}{2(D+1)}\left(\left(R_{\mathrm{final}}+1\right)^{D+1}-R_{\mathrm{start}}^{D+1}\right)\right)
\end{equation}
\begin{equation}
...
is correct, we took a dataset with $N=1500$. We clustered subsequently
subsets of that dataset to create a plot with the computation time
over $N$. The used dataset is shown in \ref{fig:Result-Iteridense-Circles}.
...
Thereby $R_{\mathrm{final}}=16$ and this value was used when measuring
the calculation times. For comparison \ref{fig:Result-Iteridense-Circles}\,(b)
...
package \emph{Clustering} of the \noun{Julia} programming language,
\cite{ClusteringDBSCAN}. The times were measured using the feature
\emph{@elapsed} of the \noun{Julia} programming language. To get reliable
values the clustering was for every algorithm performed 2000~times
in a row and the overall time was taken.
...
for \noun{Iteridense}. This test was performed by adding z-values
to the dataset so that the clusters form cylinders. DBSCAN finds the
...
the dataset by copying its points to get in total $N=7500\thinspace$points.
The result is shown in \ref{fig:Computation-time-comparison}\,(b).
The $N$ at which \noun{Iteridense} is faster than DBSCAN is now at
about $N=3700$.

\begin{figure}
\begin{centering}
\includegraphics[width=0.66\columnwidth]{clipart/Iteridense-Performance5e4}
\par\end{centering}
...
in comparison to $\mathcal{O}\left(DN^{2}\right)$ for $R_{\mathrm{final}}=10$.
($R_{\mathrm{final}}$ around 10 is a common result when clustering
in more than 3 dimensions.) The $\mathcal{O}$ of \noun{Iteridense}
is in all areas lower than $\mathcal{O}\left(DN^{2}\right)$ except
of low $N$. Therefore \noun{Iterdense} extends the parameter range
at which clustering could be performed dramatically, compared to algorithms
with $\mathcal{O}\left(DN^{2}\right)$.
...
it took about 20 cluster attempts to find an $\epsilon$ that works.
...
it provides two ways to run the clustering. For both ways there is
a clear path on how to change the algorithm's input parameters to
achieve suitable results. \noun{Iteridense} provides different options
to affect the clustering result.
...
performs clustering as good to the DBSCAN algorithm with less time
