...
For 2~dimensions the last term vanishes and $\rho_{\mathrm{cluster}}$
...
stopped (\textbf{StopResolution}). It is by default set to $\min(64,N)$,
...
\begin{equation}
\rho_{\mathrm{dataset}}=\frac{\text{total num of points}}{\text{total num of cells}}
\end{equation}
...
$P=8$, total number of points $N=16$.\\
...
$\rho_{\mathrm{cluster}}={\displaystyle \frac{P}{C}}$, $\rho_{\mathrm{dataset\,2D}}={\displaystyle \frac{2\cdot2\cdot P}{R^{2}}}$,
...
\begin{equation}
\rho_{\mathrm{cluster\,norm}}=\frac{\text{num points in cluster}}{\text{num cells of cluster}}\cdot\frac{\text{total num of points}}{\text{total num of cells}}\cdot\frac{\text{dimension}}{2\thinspace\text{resolution}^{\text{dimension}-2\vphantom{\hat{A}}}}
\end{equation}
...
\]
For $\mathcal{O}$ we can neglect the addition of $N$ and get
\begin{equation}
\mathcal{O}\left(\left(R_{\mathrm{final}}-R_{\mathrm{start}}+2\right)ND+\frac{3^{D}}{2(D+1)}\left(\left(R_{\mathrm{final}}+1\right)^{D+1}-R_{\mathrm{start}}^{D+1}\right)\right)\label{eq:O()}
\end{equation}
...
\begin{itemize}
\item \noun{Iteridense} scales roughly with $\mathcal{O}\left(DN+R^{D+1}\right)$
-- a linear increase plus an offset.
...
is correct, we took a dataset with $N=1500$ and clustered subsets
of that dataset. Every subset has a different $N$, which allows to
create a plot with the computation time over $N$. The used dataset
is shown in \ref{fig:Result-Iteridense-Circles}. It was generated
...
$N=670$. By repeating the test in 3D, we should see an increase of
this value because of the greater offset in $\mathcal{O}$ for \noun{Iteridense}.
This test was performed by adding z-values to the dataset so that
the clusters form cylinders. DBSCAN finds the 2~clusters for the
range $0.2\le\epsilon\le0.26$ and we chose the lowest possible $\epsilon=0.2$.
Eventually, we extended the dataset by multiplying its points to get
in total $N=7500\thinspace$points. The result is shown in \ref{fig:Computation-time-comparison}\,(b).
